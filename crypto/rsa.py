...
class PublicKey:
    ...
    Attributes:
        ...
        e: An integer such that 1 < e < φ(n) and e and φ(n) are coprime, where φ
...
class RSA:
    ...
    DELIMITER = '$'
    """Delimiter character that is not used in Python 3.7's Base64 encoding 
    implementation."""
    ...
    def generate(cls):
        ...
        # If we choose a random prime instead of any random number, we only need
        # to check if φ(n) is a multiple of e to verify that φ(n) and e are
        # relatively prime.
        e = 1
        while phi_n % e == 0:
            # Clearly φ(n) is at least 200 decimal digits. The database does not
            # store primes of this size, i.e., any element e in the database
            # satisfies 1 < e < φ(n).
    ...
    def encrypt(self, message: str) -> str:
        ...
        Note that all occurrences of the NULL character U+0000 will be lost in
        the encryption process.
        """
        # Get the UTF-8 byte array of the message, and convert each byte into a
        # string of 3 digits.
        ...
        transformed_numbers = [str(power(m, self.public_key.e, self.public_key.n))
                               for m in numbers]
        ...
        # Now we split each every transformed number into pairs of digits that
        # represent bytes.
        # Three-digit numbers might not be in [0, 255], but two-digit numbers
        # always will be.
        byte_arrays = [
    ...
    def decrypt(self, message: str) -> str:
        ...
        b64_arrays = [b64decode(line.encode())
                      for line in message.split(self.DELIMITER)]
        # Get the int (two-digit) representation of each byte in every byte array.
        byte_arrays = [
            [int(b) for b in b64_array]
            for b64_array in b64_arrays]
        # Concatenate each byte array to form a string representing c.
        # Single-digit numbers get pre-appended with a '0'. Convert the result
        # to an int.
        transformed_numbers = [
            int(''.join([f'{b:02}' for b in byte_array]))
            for byte_array in byte_arrays]
        # Apply c^d % n = m to each transformed number c to get the original
        # number m.
        numbers = [
            str(power(c, self.private_key.d, self.private_key.n))
            for c in transformed_numbers]
        # Split each number m (starting from right to left) into three-digit
        # numbers.
        decrypted_bytes = [
            int(m[(0 if i < 3 else i - 3): i])
            for m in numbers[::-1] for i in range(len(m), 0, -3)]
        # Reverse and remove all zeros.
        original_byte_array = [dec_byte
                               for dec_byte in decrypted_bytes[::-1]
                               if dec_byte != 0]
